...
const filterRegex = /(\<)|(\>)/g;
const commentRegex = /\n?\[center\](.*?)\[\/center\]\n?|\n?\[right\](.*?)\[\/right\]\n?|\n?\[left\](.*?)\[\/left\]\n?|\n?\[hide\](.*?)\[\/hide\]\n?|\*\*\*(.*?)\*\*\*|\*\*(.*?)\*\*|\*(.*?)\*|(\!)?\[(.*?)\]\((.*?)\.(.*?)(?:\s+width=(\d+))?(?:\s+height=(\d+))?(?:\s+width=(\d+))?\s*\)/gmi;

 // italics: *TEXT*
  // bold: **TEXT**
  // bold italics: ***TEXT***
  // img ![ALT](SRC width=NUMBER height=NUMBER)
  // link [TEXT](HREF)
...
function filterComment (match,left,right) {
  if (left)
    return '&lt;';
  if (right)
    return '&gt;';
...
function commentStyling (match,
    centerText,
    rightText,
    leftText,
    hideText,
    strongem, strong, em,
    isImg, alt, src,dot, w1, h, w2) {
  
  if (centerText)
    return '<div style="text-align: center;">'+centerText.replace(commentRegex,commentStyling)+'</div>';
  if (rightText)
    return '<div style="text-align: right;">'+rightText.replace(commentRegex,commentStyling)+'</div>';
  if (leftText)
    return '<div style="text-align: left;">'+leftText.replace(commentRegex,commentStyling)+'</div>';
  
  if (hideText)
    return `<details ontoggle="this.querySelector('summary').innerText = this.open ? 'Hide me' : 'Show me';"><summary>Show me</summary>${ hideText.replace(commentRegex,commentStyling) }</details>`;
  
  if (strongem)
    return '<strong><em>'+strongem.replace(commentRegex,commentStyling)+'</em></strong>';
  if (strong)
    return '<strong>'+strong.replace(commentRegex,commentStyling)+'</strong>';
  if (em)
    return '<em>'+em.replace(commentRegex,commentStyling)+'</em>';
  ...
  if(src) {
    if (isImg) {
      ...
      return `<img src="${src}.${dot}" alt="${alt}"${width+height}>`;
    
    } else
      return `<a target="_blank" href="${src}.${dot}">${alt}</a>`;
  }
...
const s_formId = '1FAIpQLSf-_WWfkReAszD3B-umxEUxolOtSjyumnyyf3CVN9_AXpx-XQ';
const s_nameId = '1038489309';
const s_websiteId = '889625530';
const s_textId = '1994242041';
const s_pageId = '906188187';
const s_replyId = '713483681';
const s_sheetId = '1cFFjQMXTzYoPP6Ce2pu0sbCRFszr_yx4DEsVhgWqSJc';
...
const s_commentsPerPage = 1000; // The max amount of comments that can be displayed on one page, any number >= 1 (Replies not counted)
const s_maxLength = 1000; // The max character length of a comment
const s_maxLengthName = 38; // The max character length of a name
...
const s_filterReplacement = 'âœ¨fuckâœ¨'; // Change what filtered words are censored with (**** is the default)
...
const s_noCommentsText = 'Empty :0';
const s_closedCommentsText = 'No comments right now, soz';
...
const v_mainHtml = `
    <div id="c_inputDiv">
        <form id="c_form" tabindex="-1" onsubmit="c_submitButton.disabled = true; v_submitted = true; localStorage['BLOG-comment-name'] = document.querySelector('#c_nameWrapper input').value; localStorage['BLOG-comment-website'] = document.querySelector('#c_websiteWrapper input').value;" method="post" target="c_hiddenIframe" action="https://docs.google.com/forms/d/e/${s_formId}/formResponse"></form>
    </div>
...
const v_formHtml = `
    <div id="c_nameWrapper" class="c-inputWrapper">
        ...
        <input class="c-input c-nameInput" name="entry.${s_nameId}" id="entry.${s_nameId}" type="text" maxlength="${s_maxLengthName}" required value="${localStorage['BLOG-comment-name']||''}">
    </div>
    ...
    <div id="c_websiteWrapper" class="c-inputWrapper">
        ...
        <input class="c-input c-websiteInput" name="entry.${s_websiteId}" id="entry.${s_websiteId}" type="url" pattern="https://.*" value="${localStorage['BLOG-comment-website']||''}">
    </div>

    <div id="c_textWrapper" class="c-inputWrapper">
        <details class=styling-tips>
          ...
          <dl>
            ...
            <dt> Strong / bold:
              <dd> <code>**TEXT**</code>
            <dt> Strong emphasis / bold italics:
              <dd> <code>***TEXT***</code>
            <dt> Hyperlink:
              <dd> <code>[TEXT](HREF)</code>
            <dt> Image:
              <dd> <code>![ALT](SRC width=NUMBER height=NUMBER)</code>
            <dt> Hidden / spoiler:
              <dd> <code>[spoiler]TEXT[/spoiler]</code>
            <dt> Center-aligned:
              <dd> <code>[center]TEXT[/center]</code>
            <dt> Right-aligned:
              <dd> <code>[right]TEXT[/right]</code>
            <dt> Left-aligned:
              <dd> <code>[left]TEXT[/left]</code>
          </dl>
        </details>
        <label class="c-label c-textLabel" for="entry.${s_textId}">${s_textFieldLabel}</label>
        <textarea class="c-input c-textInput" name="entry.${s_textId}" id="entry.${s_textId}" rows="4" cols="50"  maxlength="${s_maxLength}" required autocomplete=off></textarea>
    </div>
...
c_form.setAttribute('aria-labelledby','guestbook_heading');
...
function getComments() {
    ...
    retrievedSheet.then(result => {
        ...
        const json = JSON.parse(
          result.split('\n')[1]
            .replace(/google.visualization.Query.setResponse\((.*?)\);$/m, (m,p) => {return p}) 
        );
...
function createComment(data) {
    ...
    time.setAttribute('datetime',timestamps[2]);
    ...
    const filteredText = data.Text.replace(filterRegex,filterComment);
    text.innerHTML = filteredText.replace(commentRegex,commentStyling);
    ...
    signature.append(name,' ` ',time);
    
    comment.append(text,signature);
...
function convertTimestamp(timestamp) {
    const vals = timestamp.split(/\D+/);
      vals.pop();
      vals.shift();
    const date = new Date(...vals);
    const timezoneDiff = (s_timezone * 60 + date.getTimezoneOffset()) * -1;
    let offsetDate = new Date(date.getTime() + timezoneDiff * 60000);
    if (s_daylightSavings) {
      offsetDate = isDST(offsetDate)
    }
    return [offsetDate.toLocaleString(), offsetDate.toLocaleDateString(), offsetDate.toJSON()];
}


function newTimestamp (timestamp) {
  const values = timestamp.split(/\D+/);
  // yr, mo, dd, hr, mm, s
...
function openReply(id) {
    ...
    document.querySelector('#c_form').focus();
}
